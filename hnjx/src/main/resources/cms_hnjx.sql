...
CREATE TABLE IF NOT EXISTS `cont_category` (
  ...
  `parent_category_id` int(11) unsigned DEFAULT NULL COMMENT '上级类别',
...
-- 正在导出表  cms_hnjx.cont_product_info 的数据：~0 rows (大约)
DELETE FROM `cont_product_info`;
...
CREATE TABLE IF NOT EXISTS `cont_product_param` (
  `cont_product_param_id` varchar(255) DEFAULT NULL COMMENT '主键',
  `cont_product_id` int(10) unsigned DEFAULT NULL COMMENT '产品主键',
  `lang_type` varchar(15) DEFAULT NULL COMMENT '语种：字段表（CmsLangType）',
  `param_name` varchar(255) DEFAULT NULL COMMENT '参数名称',
  `limit_num` int(8) DEFAULT NULL COMMENT '限制数量',
  `create_by` bigint(255) unsigned DEFAULT NULL COMMENT '创建用户id',
  `gmt_create` datetime DEFAULT NULL COMMENT '创建时间',
  `gmt_modified` datetime DEFAULT NULL COMMENT '修改时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 正在导出表  cms_hnjx.cont_product_param 的数据：~0 rows (大约)
DELETE FROM `cont_product_param`;
...
CREATE TABLE IF NOT EXISTS `cont_product_pk` (
  `cont_product_pk_id` int(11) NOT NULL COMMENT '主键',
...
-- 正在导出表  cms_hnjx.sys_dept 的数据：~3 rows (大约)
...
) ENGINE=InnoDB AUTO_INCREMENT=125 DEFAULT CHARSET=utf8 COLLATE=utf8_bin ROW_FORMAT=DYNAMIC COMMENT='字典表';
...
CREATE TABLE IF NOT EXISTS `sys_file` (
  ...
  `create_date` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=143 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='文件上传';

-- 正在导出表  cms_hnjx.sys_file 的数据：~1 rows (大约)
...
CREATE TABLE IF NOT EXISTS `sys_log` (
  ...
  `gmt_create` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=3117 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='系统日志';

-- 正在导出表  cms_hnjx.sys_log 的数据：~71 rows (大约)
DELETE FROM `sys_log`;
