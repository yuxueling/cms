...
import com.sxyhton.common.config.Constant;
import com.sxyhton.common.domain.ColumnDO;
import com.sxyhton.common.domain.TableDO;
...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table, List<Map<String, String>> columns, ZipOutputStream zip) {
        ...
        tableDO.setTableName(table.get("tableName"));//获取表名
        tableDO.setComments(table.get("tableComment"));//获取表的备注
        //去除了表名默认的前缀并且添加了指定的表前缀(如果参数为空，则不去除)
        String className = tableToJava(tableDO.getTableName(), config.getString("tablePrefix"), config.getString("autoRemovePre"));
        tableDO.setClassName(className);//设置实体类类名
    ...
    /**
     * 去除了表默认的前缀并且添加了指定的表前缀
     * @param tableName 数据库中表的默认名称
     * @param tablePrefix 要添加的指定表前缀
     * @param autoRemovePre
     * @return
     */
    public static String tableToJava(String tableName, String tablePrefix, String autoRemovePre) {
        if (Constant.AUTO_REOMVE_PRE.equals(autoRemovePre)) {//如果设置了去除表前缀为true
            tableName = tableName.substring(tableName.indexOf("_") + 1);//去除了表前缀
        }
        if (StringUtils.isNotBlank(tablePrefix)) {//不是空白的情况
